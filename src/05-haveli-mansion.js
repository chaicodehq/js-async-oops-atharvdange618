...
export class HaveliSecurity {
  ...
  addResident(name, role, passcode) {
    ...
    if (this.#residents.some((res) => res.name === name)) {
  ...
  removeResident(name, passcode) {
    ...
    const index = this.#residents.findIndex((res) => res.name === name);
    if (index === -1) {
  ...
  verifyAccess(name) {
    const isResident = this.#residents.some((res) => res.name === name);
    this.#accessLog.push({
      name,
  ...
  isResident(name) {
    return this.#residents.some((res) => res.name === name);
  }
}
